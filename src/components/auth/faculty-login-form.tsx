...
import { loginFacultyPortal } from "@/lib/actions/auth";
...
const loginErrorKeyMap = {
  ...
  FACULTY_ONLY: "Use the faculty login portal",
} as const;
...
export function FacultyLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await loginFacultyPortal(staffUsername.trim(), password);
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-icvf-navy sm:text-3xl">Faculty Portal</h1>
