...
// Faculty portal clone of announcement-thread.tsx. The teacher-side
// component hardcodes its server-action imports (addAnnouncementReply etc.
// from src/lib/actions/announcements), so it isn't reusable as-is for the
// fully-isolated faculty_* tables — this variant is identical except it
// calls the faculty-announcements actions and treats "faculty_staff" as the
// staff role instead of "teacher".

import { useCallback, useEffect, useState } from "react";
import { GlassCard } from "@/components/shared/glass-card";
import { EmptyState } from "@/components/shared/empty-state";
import {
  addFacultyAnnouncementReply,
  deleteFacultyAnnouncementReply,
  getFacultyAnnouncementAttachmentUrl,
  getFacultyAnnouncementThread,
} from "@/lib/actions/faculty-announcements";
...
const STAFF_ROLES: UserRole[] = ["faculty_staff", "admin", "super_admin"];
...
export function FacultyAnnouncementThread({ announcementId }: { announcementId: string }) {
  const { user } = useAuth();
  ...
  const refresh = useCallback(async () => {
    try {
      const thread = await getFacultyAnnouncementThread(announcementId);
  ...
  async function handleReply() {
    setSending(true);
    const result = await addFacultyAnnouncementReply(announcementId, reply);
    setSending(false);
    if (!result.ok) {
  ...
  async function handleDeleteReply(replyId: string) {
    const result = await deleteFacultyAnnouncementReply(replyId);
    if (!result.ok) {
      toast.error(result.error);
      return;
    }
  ...
  async function handleOpenAttachment() {
    const result = await getFacultyAnnouncementAttachmentUrl(announcementId);
    if (!result.ok) {
      toast.error(result.error);
      return;
    }
