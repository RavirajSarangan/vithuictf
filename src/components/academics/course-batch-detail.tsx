...
export function CourseBatchDetail({
  batchId: id,
  ...
  const sessionsThisWeek = useMemo(() => {
    ...
    return sessions.filter(
      (s) => s.status === "scheduled" && s.scheduledDate >= start && s.scheduledDate <= end
    ).length;
  }, [sessions]);

  const attendanceRate = useMemo(() => {
    const attended = attendanceSummary.reduce((sum, r) => sum + r.present + r.late, 0);
    const marked = attendanceSummary.reduce((sum, r) => sum + r.present + r.late + r.absent, 0);
    return marked > 0 ? Math.round((attended / marked) * 100) : null;
  }, [attendanceSummary]);
  ...
  const handleRemove = async (enrollmentId: string) => {
    ...
    try {
      ...
      toast.error(getActionErrorMessage(e, "Remove failed"));
    }
  };
  ...
  const handleArchive = async () => {
    try {
      ...
      toast.error(getActionErrorMessage(e, "Archive failed"));
    }
  };
  ...
  const handleReactivate = async () => {
    try {
      ...
      toast.error(getActionErrorMessage(e, "Reactivate failed"));
    }
  };
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      toast.error(getActionErrorMessage(e, "Regenerate failed"));
    } finally {
  ...
  if (!batch) {
    return <p className="text-sm text-muted-foreground">Course schedule not found.</p>;
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <PageHeader
        ...
        description={`${batch.batchCode} · ${batch.startDate} to ${batch.endDate} · ${batch.totalClasses} planned classes`}
        action={
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" onClick={() => setEditOpen(true)}>
              <Pencil className="mr-2 size-4" /> Edit schedule
            </Button>
            {batch.active ? (
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold">{enrollLoading ? "—" : activeEnrollmentCount}</p>
            <p className="text-xs text-muted-foreground">
              {enrollLoading ? "" : `${enrollments.length} total enrollment(s)`}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold">{sessionsLoading ? "—" : sessionsThisWeek}</p>
            <p className="text-xs text-muted-foreground">
              {sessionsLoading ? "" : `${sessionStats.remaining} remaining overall`}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold">
              {sessionsLoading ? "—" : `${sessionStats.completed}/${sessionStats.total}`}
            </p>
            <p className="text-xs text-muted-foreground">
              {sessionsLoading ? "" : `${sessionStats.cancelled} cancelled`}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {attendanceRate === null ? "No attendance marked yet" : "Across marked sessions"}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <BatchMessageDialog
        batchId={id}
        batchName={batch.courseName ?? batch.name}
      ...
      <section id="enrolled" className="space-y-3">
        ...
        {enrollLoading ? (
          ...
          <AdminTableSection
            ...
            data={enrollments}
            summaryItems={genericTableSummary(enrollments, "Enrolled")}
            entityLabel="enrollment"
            exportConfig={{
              filename: `${batch?.batchCode ?? "course"}-enrollments.csv`,
              columns: [
                { key: "enrollmentCode", label: "Enrollment ID" },
                { key: "studentName", label: "Name" },
      ...
      {isAdmin ? (
        <section className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            Recent automated and manual WhatsApp messages for this course.
          </p>
          {whatsappLogLoading ? (
            ...
            <AdminTableSection
              ...
              data={whatsappLog}
              summaryItems={genericTableSummary(whatsappLog, "Messages")}
              entityLabel="message"
              exportConfig={{
                columns: [
