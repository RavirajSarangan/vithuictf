...
// Faculty portal clone of src/lib/actions/quizzes.ts — MCQ practice quizzes
// scoped to the faculty_staff role and faculty_batches instead of
// teacher/course_batches. Every query targets the faculty_quizzes /
// faculty_quiz_questions / faculty_quiz_attempts tables (see
// 20260726095000_faculty_quizzes.sql); every staff gate is
// requireFacultyStaff()/requireFacultyFeatureAccess() instead of
// requireAcademicsStaff()/requireFeatureAccess(). RLS
// (faculty_staff_can_access_quiz, faculty_can_access_course) does the actual
// per-staff scoping; these actions just need to target the right tables.
//
// Security model for the student-facing functions (getFacultyQuizForTaking /
// submitFacultyQuizAttempt) is cloned EXACTLY from the original: the admin
// (service-role) client fetches questions WITH the correct answer
// server-side and writes the attempt row directly, so RLS never has to
// expose correct_index to the client and scores can't be forged.
...
import { mapFacultyQuiz, mapFacultyQuizAttempt, mapFacultyQuizQuestion } from "@/lib/supabase/mappers";
import type { Quiz, QuizAttempt, QuizQuestion } from "@/types";

function revalidateFacultyQuizPaths() {
  safeRevalidatePath("/faculty/quizzes");
}
...
export async function getFacultyStaffQuizzes(): Promise<Quiz[]> {
  await requireFacultyStaff();
  const supabase = await createClient();

  const { data: rows, error } = await supabase
    .from("faculty_quizzes")
    .select("*, courses(name), faculty_batches(name)")
    .order("created_at", { ascending: false });
  if (error) throw new Error(error.message);

  const quizzes = (rows ?? []).map(mapFacultyQuiz);
  if (!quizzes.length) return quizzes;

  const ids = quizzes.map((q) => q.id);
  const [{ data: questionRows }, { data: attemptRows }] = await Promise.all([
    supabase.from("faculty_quiz_questions").select("quiz_id, points").in("quiz_id", ids),
    supabase.from("faculty_quiz_attempts").select("quiz_id").in("quiz_id", ids),
  ]);
...
export async function getFacultyQuizEditor(quizId: string): Promise<{
  quiz: Quiz;
  ...
  const [{ data: questionRows }, { data: attemptRows }] = await Promise.all([
    supabase
      .from("faculty_quiz_questions")
      .select("*")
      .eq("quiz_id", quizId)
      .order("position", { ascending: true }),
    supabase
      .from("faculty_quiz_attempts")
  ...
  return {
    quiz: mapFacultyQuiz(quizRow),
    questions: (questionRows ?? []).map((row) => mapFacultyQuizQuestion(row)),
    attempts: (attemptRows ?? []).map(mapFacultyQuizAttempt),
  };
}

export async function createFacultyQuiz(data: {
  courseId: string;
  batchId?: string | null;
  title: string;
  description: string;
  ...
  try {
    const profile = await requireFacultyFeatureAccess("quizzes_exams");
    const supabase = await createClient();
...
export async function updateFacultyQuiz(
  quizId: string,
  data: {
    title: string;
    description: string;
...
export async function deleteFacultyQuiz(quizId: string): Promise<ActionResult> {
  try {
    await requireFacultyStaff();
    const supabase = await createClient();

    const { error } = await supabase.from("faculty_quizzes").delete().eq("id", quizId);
...
export async function saveFacultyQuizQuestion(input: {
  id?: string;
  quizId: string;
  prompt: string;
  options: string[];
  ...
  try {
    ...
    if (input.id) {
      ...
      const { error } = await supabase.from("faculty_quiz_questions").insert({
...
export async function deleteFacultyQuizQuestion(questionId: string): Promise<ActionResult> {
  try {
    await requireFacultyStaff();
    const supabase = await createClient();

    const { error } = await supabase.from("faculty_quiz_questions").delete().eq("id", questionId);
...
export async function getFacultyQuizForTaking(quizId: string): Promise<
...
> {
  try {
    ...
    return {
      ok: true,
      quiz: mapFacultyQuiz(quizRow),
      questions: questionRows.map((row) => mapFacultyQuizQuestion(row, { includeAnswer: false })),
...
export async function submitFacultyQuizAttempt(
  quizId: string,
...
> {
  try {
    ...
    const { error: insertError } = await admin.from("faculty_quiz_attempts").insert({
      quiz_id: quizId,
