...
import { createAssignment, getStaffAssignments } from "@/lib/actions/assignments";
...
export default function AcademicsAssignmentsPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      setAssignments(await getStaffAssignments());
    } catch (error) {
                ...
                {assignments.map((assignment) => (
                  <tr key={assignment.id} className="border-b last:border-0 hover:bg-muted/40">
                    <td className="px-4 py-3">
                      <Link
                        href={`/academics/assignments/${assignment.id}`}
