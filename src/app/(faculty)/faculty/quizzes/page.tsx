...
// Faculty portal clone of src/app/(academics)/academics/quizzes/page.tsx —
// same list + create-dialog UX, targeting the faculty_quizzes action/hook
// set instead of the teacher-side one. Course picker reuses useAdminCourses
// (courses are globally readable — see courses_select_auth RLS policy) the
// same way the teacher-side page does; the batch picker uses
// useFacultyBatches, which RLS already scopes to this faculty member's
// courses. If a faculty member picks a course they don't manage, quiz
// creation is still blocked server-side by faculty_quizzes' RLS
// (faculty_can_access_course), matching how faculty batch creation behaves
// today.
...
import { createFacultyQuiz, getFacultyStaffQuizzes } from "@/lib/actions/faculty-quizzes";
...
export default function FacultyQuizzesPage() {
  ...
  const refresh = useCallback(async () => {
    try {
      setQuizzes(await getFacultyStaffQuizzes());
    } catch (error) {
  ...
  async function handleCreate() {
    ...
    setSubmitting(true);
    const result = await createFacultyQuiz({
      courseId,
    ...
    setDialogOpen(false);
    router.push(`/faculty/quizzes/${result.id}`);
  }
